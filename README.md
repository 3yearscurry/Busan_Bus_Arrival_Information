# [공지] 버스 정보 조회 지연 및 504 오류 안내

현재 버스 도착 정보, 노선 및 정류장 조회 과정에서 서버 응답이 지연되거나 `504 Gateway Timeout` 오류가 발생하고 있습니다.
확인 결과, 서버와 부산 버스 API 간 통신 과정에서 응답 시간이 초과되는 문제로 파악되었습니다. 클라이언트의 문제는 아니며, 현재 원인에 맞춰 서버 연결을 점검하고 있습니다.
영향 범위: 버스 도착 정보, 노선 검색, 정류장 검색
발생 증상: 조회 지연, 일시적인 서버 연결 오류 또는 504 오류

이용에 불편을 드려 죄송합니다. 정상화되는 대로 다시 안내드리겠습니다.
